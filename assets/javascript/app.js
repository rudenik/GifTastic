...
var weatherQueryURL =
  "https://api.openweathermap.org/data/2.5/forecast?APPID=dcfef77b1fe26edc9d499d914dee01c8&q=";
...
function checkForFavourites() {
  ...
  if (favGifs.length > 0 && $(".btn-group").children().length == 3) {
    ...
    var favLabel = $(
      "<label class='btn btn-secondary' id='removableFavButton'>"
    );
    var favInput = $(
      "<input type='radio' name='options' id='Favourites' autocomplete='off'>"
    );
  ...
  for (favs in favGifs) {
    $.ajax({
      ...
      cardBody
        .addClass("card-body")
        .attr("id", "clickToGif")
        .attr("data-animated", resp["data"]["images"].fixed_width["url"]);
      var cardText = $("<p>");
      ...
      var imageToAdd = $("<img>");
      imageToAdd
        .attr("src", resp["data"]["images"].fixed_width_still["url"])
        .attr("data-animated", resp["data"]["images"].fixed_width["url"])
        .attr("alt", "still image for gif " + resp["data"].title);
...
function makeRequest(term, oset) {
  ...
  switch (selectedAPI) {
    case "Giphy":
      ...
      searchTermDiv.html(
        "<b>Here are the results for " + queryTerm + " on Giphy</b>"
      );
      ...
      $.ajax({
        ...
        for (elements in resp["data"]) {
          ...
          cardBody
            ...
            .attr(
              "data-animated",
              resp["data"][elements]["images"].fixed_width["url"]
            );
          var cardText = $("<p>");
          cardText.addClass("card-text");
          ...
          imageToAdd
            .attr(
              "src",
              resp["data"][elements]["images"].fixed_width_still["url"]
            )
            .attr(
              "data-animated",
              resp["data"][elements]["images"].fixed_width["url"]
            )
            .attr("alt", "still image for gif " + resp["data"][elements].title);
          imageToAdd.addClass("card-img-bottom");
          var favImg = $("<img src='assets/images/starempty.png'>");
          favImg
            .attr("id", "favImg")
            .attr("data-imageID", resp["data"][elements]["id"]);
          var dlButton = $("<button>");
          dlButton.text("Download");
          dlButton.attr("id", "dlbutton").addClass("btn btn-outline-secondary");
          cardBody.append(cardText);
    ...
    case "Weather":
      ...
      searchTermDiv.attr("id", "searchtermdiv");
      searchTermDiv.html(
        "<b>Here is the five day forcast for " + queryTerm + "</b>"
      );
      searchTermDiv.prependTo($("#results-col"));
      $.ajax({
        url:
          "https://api.openweathermap.org/data/2.5/forecast?APPID=dcfef77b1fe26edc9d499d914dee01c8&units=metric&q=" +
          term,
        method: "GET"
      }).then(function(resp) {
        for (elements in resp["list"]) {
          var weatherObj = {
            ...
            icon:
              ...
              resp["list"][elements]["weather"][0].icon +
              ".png"
          };
        ...
        for (dates in weatherArray) {
          var arrayDateNumber = parseInt(
            weatherArray[dates].dateTime.substring(8, 10)
          );
        ...
        for (DA in dayArray) {
          if (dayArray[DA].length == 0) {
            ...
            cardHeader.html(
              "<b>" + dayArray[DA][0].dateTime.substring(0, 11) + "</b>"
            );
            ...
            for (ele in dayArray[DA]) {
              ...
              listItem.html(
                "<br>Time: " +
                  dayArray[DA][ele].dateTime.substring(10, 16) +
                  "<br>Temp: " +
                  dayArray[DA][ele].temp +
                  "<br>Pressure: " +
                  dayArray[DA][ele].pressure
              );
    ...
    case "OMDB":
      ...
      searchTermDiv.attr("id", "searchtermdiv");
      searchTermDiv.html(
        "<b>Here are the results for " + queryTerm + " on OMDB</b>"
      );
      searchTermDiv.prependTo($("#results-col"));
      $.ajax({
        url: (omdbQueryURL =
          "https://www.omdbapi.com/?apikey=8252a0f9&s=" + term),
        method: "GET"
      }).then(function(resp) {
        ...
        for (posMov in movieResults) {
          ...
          cardDiv
            ...
            .attr("id", "moviecard")
            .attr("data-movie", movieResults[posMov].title)
            .attr("data-poster", movieResults[posMov].poster)
            .attr("data-poster-showing", "true");
          var cardHeader = $("<div>");
          cardHeader.addClass("card-header");
          cardHeader.html(
            "<b>Title: </b>" +
              movieResults[posMov].title +
              "<br>Year: " +
              movieResults[posMov].year
          );
          cardHeader.appendTo(cardDiv);
          var imageToAdd = $("<img>");
          imageToAdd
            .attr("src", movieResults[posMov].poster)
            .attr("alt", movieResults[posMov].title + " Poster");
          imageToAdd.addClass("card-img-bottom");
...
$(".container-fluid").on("click", "#moviecard", function() {
  ...
  if ($(this).attr("data-poster-showing") == "true") {
    $.ajax({
      url: (omdbQueryURL =
        "https://www.omdbapi.com/?apikey=8252a0f9&t=" + term),
      method: "GET"
    }).then(function(resp) {
      actors = resp["Actors"];
      awards = resp["Awards"];
      ...
      cardText.addClass("card-text");
      cardText.html(
        "<b>Actors: </b>" + actors + "<br><b>Awards: </b>" + awards
      );
